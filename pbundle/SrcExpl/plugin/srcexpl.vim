...
function! g:SrcExpl_GoToCurrent()
...
function! g:SrcExpl_Refresh()
	...
	if "quickfix" == &buftype || "qf" == &filetype
		if !<SID>SrcExpl_GoQuickfix()
...
function! <SID>SrcExpl_WinNew(wincmd)
	...
	exe 'silent! wincmd b'
    exe 'silent! belowright ' . string(g:SrcExpl_winHeight) . 'split ' . a:wincmd
...
function! <SID>SrcExpl_WinEnter()
    ...
    elseif <SID>SrcExpl_AdaptPlugins()
        ...
        if maparg("<2-LeftMouse>", "n") == ":call g:SrcExpl_Jump()<CR>"
            nunmap <silent> <2-LeftMouse>
        endif
        " Unmap the jump key
        if maparg(g:SrcExpl_jumpKey, 'n') == ":call g:SrcExpl_Jump()<CR>"
    ...
    " In the edit window
        ...
        if has("gui_running")
            ...
            silent! nnoremenu 1.01 PopUp.&SrcExplGoBack :call g:SrcExpl_GoBack()<CR>
        endif
        " Map the user's key to go back from the definition context
        if g:SrcExpl_gobackKey != ""
            exe "nnoremap " . g:SrcExpl_gobackKey . " :call g:SrcExpl_GoBack()<CR>"
        endif
        " Unmap the exact mapping of 'double-click'
        if maparg("<2-LeftMouse>", "n") == ":call g:SrcExpl_Jump()<CR>"
            nunmap <silent> <2-LeftMouse>
...
function! <SID>SrcExpl_GoQuickfix()
	let line = getline(".")
	let pat = "|[0-9]*|"
	let b = match(line, pat)
	let e = matchend(line, pat)
	if b != -1 && e != -1
		let l:filename = strpart(line, 0, b)
		let l:num = strpart(line, b+1, e-b-2)
		call <SID>SrcExpl_WinEdit(l:filename)
		if <SID>SrcExpl_WinActive()
			call cursor(l:num, 1)
			normal zt
			call <SID>SrcExpl_MatchExpr()
...
function! <SID>SrcExpl_GoPy(expr)
	let l:list = jedi#pyeval("jedi_vim.get_goto(mode='goto')")
	let l:len = len(l:list)
	if l:len > 0
		let l:item = l:list[0]
		...
		" echo l:item
		if <SID>SrcExpl_WinActive()
			call cursor(l:item['line'], l:item['col'])
			normal zt
			call <SID>SrcExpl_MatchExpr()
			call <SID>SrcExpl_ColorExpr()
			call <SID>SrcExpl_SetCurrMark()
	...
	call <SID>SrcExpl_WinPrompt('Python Definition Not Found')
	let s:SrcExpl_status = 0
	return -1
endfunction " }}}
...
function! <SID>SrcExpl_CleanUp()
    ...
    if maparg(g:SrcExpl_updateTagsKey, 'n') == ":call g:SrcExpl_UpdateTags()<CR>"
        exe "nunmap " . g:SrcExpl_updateTagsKey
    endif

    " Unmap the previous key
    if maparg(g:SrcExpl_prevDefKey, 'n') == ":call g:SrcExpl_PrevDef()<CR>"
        exe "nunmap " . g:SrcExpl_prevDefKey
    endif

    " Unmap the next key
    if maparg(g:SrcExpl_nextDefKey, 'n') == ":call g:SrcExpl_NextDef()<CR>"
...
function! <SID>SrcExpl_Init()
    ...
    if g:SrcExpl_updateTagsKey != ""
        exe "nnoremap " . g:SrcExpl_updateTagsKey . " :call g:SrcExpl_UpdateTags()<CR>"
    endif

    if g:SrcExpl_prevDefKey != ""
        exe "nnoremap " . g:SrcExpl_prevDefKey . " :call g:SrcExpl_PrevDef()<CR>"
    endif

    if g:SrcExpl_nextDefKey != ""
        exe "nnoremap " . g:SrcExpl_nextDefKey . " :call g:SrcExpl_NextDef()<CR>"
    endif
...
function! g:SrcExpl_ToggleActiveSrcExpl()
	if <SID>SrcExpl_WinActive()
		if exists('s:SrcExpl_editWin') && s:SrcExpl_editWin != 0
			silent! exe s:SrcExpl_editWin . "wincmd w"
		endif
	else
		call <SID>SrcExpl_WinGo()
