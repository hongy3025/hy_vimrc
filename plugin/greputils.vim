...
function! s:Grep(cmdType, preview, grepAdd, ...)
  ...
  try
    ...
    if (g:greputils{substitute(a:cmdType, '.', '\U&', '')}CompMode != 'file')
      if (fileArgs !~ '^\s*$')
        let fileArgs = genutils#UserFileExpand(fileArgs)
      endif
    ...
    let _grepOpts = genutils#EscapeCommand('', grepOpts, fileArgs)
    let handleMultipleID = 0
    " On windows, we need to manually handle multiple ID files.
    if cmdType ==# 'lid' && genutils#OnMS()
      if genutils#GetShellEnvType() == g:ST_WIN_CMD
    ...
    let grepOpts = genutils#Escape(_grepOpts, '%#!')
    ...
    while 1
      ...
      if handleMultipleID
        if genutils#GetShellEnvType() == g:ST_WIN_CMD
          ...
          let addOpts = genutils#EscapeCommand('', '-f '.MvIterNext('GrepUtils'), ' ')
        endif
      endif
      ...
      if !a:preview
        ...
        exec genutils#MakeArgumentList('argumentList', ' ')
...
function! s:InitPreviewWindow(grepAdd, cmd, args)
  if a:grepAdd && g:greputilsPreviewCreateFolds
    if IsPositionSet('GrepUtils')
      call genutils#RestoreHardPosition('GrepUtils')
    endif
  ...
  call genutils#SilentSubstitute('^\(\%(\f\| \)\+\):\(\d\+\):', '%s//\1|\2|/e')
...
function! s:_VimGrep(fline, lline, inverse, grepAdd, vimWild, ...)
  ...
  if a:vimWild ==# 'windo' && prvWinnr != -1
    ...
    call genutils#CloseWindow(prvWinnr, 1)
  else
  ...
  try
    ...
    let result = genutils#GetVimCmdOutput(cmd.
  ...
  call genutils#SilentDelete(',$', '^E486:')
  call genutils#SilentDelete(',$', '\d\+ lines, \d\+ characters$')
  call genutils#SilentDelete(',$', '\d\+ lines --\d\+%--')
  call genutils#SilentDelete(',$', '^$')
...
function! s:SubCmdType(cmdType)
  if a:cmdType ==# 'grep'
    if genutils#OnMS() && &grepprg =~? '\<findstr\>'
      return 'findstr'
    else
      return 'gnuGrep'
    endif
...
function! s:GWindow(type, size)
  ...
  if ! open
    call genutils#CloseWinNoEa(winnr(), 1)
...
function! s:OpenGrepPreview(reopen, grepAdd, size)
  ...
  if !genutils#IsOnlyVerticalWindow() && !wasVisible
  ...
  if !a:reopen
    if a:grepAdd
      $
      call genutils#SaveHardPosition('GrepUtils')
...
function! s:GNextImpl(forward, cnt)
  ...
  if ! previewWasVisible
    call genutils#CloseWindow(previewWinNr, 1)
  endif
endfunction
  ...
  if a:filterArgs != ''
    ...
    while MvIterHasNext('GrepSet')
      let filterCmd = filterCmd . '| ' .
            \ genutils#EscapeCommand(g:greputilsFiltercmd, MvIterNext('GrepSet'), '')
      let filterCmd = genutils#Escape(filterCmd, '%#!')
    endwhile
